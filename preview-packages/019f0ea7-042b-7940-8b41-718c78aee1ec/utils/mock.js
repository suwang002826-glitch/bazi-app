...
function getElementProducer(targetElement) {
  return Object.entries(elementGenerates).find(([, generated]) => generated === targetElement)?.[0] || targetElement;
}

function choosePrimaryLine(lines) {
  return [...lines].sort((left, right) => {
    const leftScore = scoreSeasonState(left.seasonState).score + (left.moving ? 2 : 0) + (left.role === '世' ? 1 : 0);
    const rightScore = scoreSeasonState(right.seasonState).score + (right.moving ? 2 : 0) + (right.role === '世' ? 1 : 0);
    return rightScore - leftScore;
  })[0] || null;
}

function formatLineBrief(line) {
  if (!line) return '未现';
  return `${line.position}${line.role ? `(${line.role})` : ''}${line.relation}${line.stem}${line.branch}${line.element}`;
}

function buildGodRoleProfile(lines, focus, focusLine, baseElement) {
  const useElement = focusLine.element;
  const originalElement = getElementProducer(useElement);
  const hostileElement = elementControlledBy[useElement] || useElement;
  const revengeElement = getElementProducer(hostileElement);
  const configs = [
    {
      key: 'use',
      title: '用神',
    ...
    },
    {
      key: 'original',
      title: '原神',
      relation: getSixRelation(baseElement, originalElement),
      element: originalElement,
      role: '生扶用神',
      rule: `${originalElement}生${useElement}`
    },
    {
      key: 'hostile',
      title: '忌神',
      relation: getSixRelation(baseElement, hostileElement),
      element: hostileElement,
      role: '克制用神',
      rule: `${hostileElement}克${useElement}`
    },
    {
      key: 'revenge',
      title: '仇神',
      relation: getSixRelation(baseElement, revengeElement),
      element: revengeElement,
      role: '生忌克原',
      rule: `${revengeElement}生${hostileElement}、克${originalElement}`
    }
  ];

  const roles = configs.map((config) => {
    const matched = config.key === 'use'
      ? lines.filter((line) => line.relation === focus)
      : lines.filter((line) => line.element === config.element);
    const primary = config.key === 'use' ? focusLine : choosePrimaryLine(matched);
    const power = primary ? scoreSeasonState(primary.seasonState) : { score: -5, factors: ['伏藏'] };
    const hasMoving = matched.some((line) => line.moving);
    return {
      ...config,
      status: matched.length ? '明现' : '伏藏',
      count: matched.length,
      primaryText: formatLineBrief(primary),
      powerLabel: primary ? getLinePowerLabel(power.score + (hasMoving ? 1 : 0)) : '伏藏',
      factors: primary ? power.factors.concat(hasMoving ? ['发动'] : []) : ['本卦不现，需看伏神飞神'],
      lineTexts: matched.map(formatLineBrief),
      text: matched.length
        ? `${config.title}${config.status}，主看${formatLineBrief(primary)}，${primary.seasonState === '平' ? '月日平常' : `见${primary.seasonState}`}。`
        : `${config.title}本卦未现，需借本宫伏神并看飞神生克。`
    };
  });

  const original = roles.find((item) => item.key === 'original');
  const hostile = roles.find((item) => item.key === 'hostile');
  const judgement = [
    original && original.status === '明现' ? `原神${original.powerLabel}，用神有来源。` : '原神不明，事情助力不足。',
    hostile && hostile.status === '明现' ? `忌神${hostile.powerLabel}，需防克用。` : '忌神不明，直接阻力暂不重。',
    original && hostile && original.status === '明现' && hostile.status === '明现' ? '原忌同现时，要看谁得月日、谁发动。' : '原忌不齐，先以用神旺衰和飞伏为主。'
  ];
...
function describeMonthDayEffect(line, branch, label) {
  const sourceElement = branchElements[branch];
  if (line.branch === branch) return `${label}临${line.branch}`;
  if (describeBranchInteraction(line.branch, [branch]).includes('冲')) return `${label}冲${line.branch}`;
  if (describeBranchInteraction(line.branch, [branch]).includes('合')) return `${label}合${line.branch}`;
  if (sourceElement === line.element) return `${label}比扶`;
  if (elementGenerates[sourceElement] === line.element) return `${label}生扶`;
  if (elementControls[sourceElement] === line.element) return `${label}克制`;
  if (elementGenerates[line.element] === sourceElement) return `爻生${label}`;
  return `${label}平`;
}

function buildMonthDayPowerProfile(lines, monthBranch, dayBranch, voidBranches) {
  const rows = lines.map((line) => {
    const season = scoreSeasonState(line.seasonState);
    const score = season.score + (line.moving ? 2 : 0) + (line.role === '世' ? 1 : 0);
    const priority = [];
    if (line.branch === monthBranch) priority.push('月建');
    if (line.seasonState.includes('月破')) priority.push('月破');
    if (line.branch === dayBranch) priority.push('日辰');
    if (line.seasonState.includes('日冲')) priority.push('日冲');
    if (line.moving) priority.push('动爻');
    if (voidBranches.includes(line.branch)) priority.push('旬空');
    return {
      ...
      label: formatLineBrief(line),
      monthEffect: describeMonthDayEffect(line, monthBranch, '月建'),
      dayEffect: describeMonthDayEffect(line, dayBranch, '日辰'),
      score,
      powerLabel: getLinePowerLabel(score),
      priority: priority.length ? priority : ['平'],
      text: `${formatLineBrief(line)}：${describeMonthDayEffect(line, monthBranch, '月建')}，${describeMonthDayEffect(line, dayBranch, '日辰')}，${priority.length ? priority.join('、') : '平'}。`
    };
  });
  return {
    rows,
    focusRows: [...rows].sort((left, right) => Math.abs(right.score) - Math.abs(left.score)).slice(0, 3),
    text: '月建为一月提纲，日辰为当日主事；先看临月、月破、临日、日冲，再参动爻与旬空。'
  };
}
...
function buildHexagramAnalysis(lines, changedLineDetails, focus, focusLine, movingLines, baseElement, voidBranches, monthBranch, dayBranch, fuShenList = []) {
  ...
  const focusFuShen = fuShenList.find((item) => item.relation === focus);
  const fuFei = focusLines.length
    ? {
        status: '用神已现',
        items: fuShenList,
        text: `${focus}在本卦中明现${focusLines.length}处，先以${focusLine.position}为主；其余伏神为${fuShenList.length ? fuShenList.map((item) => item.text).join('；') : '无明显缺位六亲'}。`
      }
    : focusFuShen
      ? {
          status: '用神伏藏',
          items: fuShenList,
          text: `本卦未见${focus}明现，${focusFuShen.text}。需看飞神是否生扶或克制伏神，再结合出伏、出空之时。`
        }
      : {
          status: '飞伏已列',
          items: fuShenList,
          text: fuShenList.length
            ? `本卦缺失六亲已从${fuShenList[0].sourcePalace}宫伏神中补参：${fuShenList.map((item) => item.text).join('；')}。`
            : '本卦五类六亲俱现，暂不另取伏神。'
        };
  ...
  const godRoles = buildGodRoleProfile(lines, focus, focusLine, baseElement);
  const monthDayPower = buildMonthDayPowerProfile(lines, monthBranch, dayBranch, voidBranches);
  ...
  return { useGod, useGodStrength, godRoles, monthDayPower, shiYing, moving, movingPriority, fuFei, riskSummary, yingQi, decision };
}

function buildYaoLine(raw, index, hex, dayStem, shiIndex, yingIndex, relationBaseElement) {
  const meta = getLineMeta(raw);
  const baseElement = relationBaseElement || hex.palace.element;
...
function buildPalaceFuShenProfile(lineDetails, focus, baseHex, dayStem, shiIndex, yingIndex, seed) {
  ...
  const palaceLines = pureLines.map((value, index) => buildYaoLine(value, index, pureHex, dayStem, shiIndex, yingIndex, baseHex.palace.element));
...
function buildFuShenList(lineDetails, baseHex, dayStem, shiIndex, yingIndex, focusRelation) {
  const existingRelations = new Set(lineDetails.map((line) => line.relation));
  const pureLines = getPalacePureLines(baseHex.palace.name);
  const pureHex = getHexagram(pureLines);
  const palaceLines = pureLines.map((value, index) => buildYaoLine(value, index, pureHex, dayStem, shiIndex, yingIndex, baseHex.palace.element));
  const line = palaceLines.find((item) => item.relation === focusRelation && !existingRelations.has(item.relation))
    || palaceLines.find((item) => !existingRelations.has(item.relation));
  if (!line) return [];
  const fly = lineDetails[line.index] || lineDetails[0];
  return [{
    ...
    element: line.element,
    naText: line.naText,
    anchor: fly ? fly.position : line.position,
    flyText: fly ? fly.naText : '',
    flyRelation: fly ? fly.relation : '',
    sourcePosition: line.position,
    sourcePalace: baseHex.palace.name,
    shownInCurrentHexagram: false,
    text: `${line.relation}${line.stem}${line.branch}${line.element}伏于${fly ? fly.position : line.position}，飞神为${fly ? fly.naText : '未定'}`
  }];
}
...
function buildHexagram(question, lines = [], category, method, castTime) {
  ...
  const changedLineDetails = changedLines.map((value, index) => {
    const line = buildYaoLine(value, index, changedHex, dayPillar.stem, shiIndex, yingIndex, baseElement);
    return {
      ...line,
      seasonState: getLineSeasonState(line.branch, monthPillar.branch, dayPillar.branch, voidBranches)
    };
  ...
  const fuShenList = buildFuShenList(lineDetails, baseHex, dayPillar.stem, shiIndex, yingIndex, focus);
  const analysis = buildHexagramAnalysis(lineDetails, changedLineDetails, focus, focusLine, movingLines, baseElement, voidBranches, monthPillar.branch, dayPillar.branch, fuShenList);
  ...
  const displayLines = lineDetails.map((line, index) => ({
    ...
    fuShen: fuShenList.find((item) => item.targetIndex === line.index) || null,
    fuShens: fuShenList.filter((item) => item.targetIndex === line.index),
    fuShenText: fuShenList.filter((item) => item.targetIndex === line.index).map((item) => item.naText).join('　'),
